        ...
        for (jj in 1:2) {
            for (kk in 1:2) {
                ...
                for (n in 1:n_sim) {
                    ...
                    lof3 <- left_join(lof2, post_betas_linreg, by="V1")
                    mis3 <- left_join(mis2, post_betas_linreg, by="V1")
                    syn3 <- left_join(syn2, post_betas_linreg, by="V1")
                    all3 <- left_join(all2, post_betas_linreg, by="V1")
                    ...
                    lof4 <- left_join(lof3, post_betas_marginal, by="V1")
                    mis4 <- left_join(mis3, post_betas_marginal, by="V1")
                    syn4 <- left_join(syn3, post_betas_marginal, by="V1")
                    all4 <- left_join(all3, post_betas_marginal, by="V1")
                    ...
                    lof5 <- left_join(lof4, post_betas_ridge, by="V1")
                    mis5 <- left_join(mis4, post_betas_ridge, by="V1")
                    syn5 <- left_join(syn4, post_betas_ridge, by="V1")
                    all5 <- left_join(all4, post_betas_ridge, by="V1")
                    ...
                    colnames(lof5) <- c("SNP", "True", "Bayes", "Linreg", "Marginal", "Ridge")
                    colnames(mis5) <- c("SNP", "True", "Bayes", "Linreg", "Marginal", "Ridge")
                    colnames(syn5) <- c("SNP", "True", "Bayes", "Linreg", "Marginal", "Ridge")
                    colnames(all5) <- c("SNP", "True", "Bayes", "Linreg", "Marginal", "Ridge")
                    write.table(lof5, betas_lof_name, row.names=F, col.names=F, quote=F)
                    write.table(mis5, betas_mis_name, row.names=F, col.names=F, quote=F)
                    write.table(syn5, betas_syn_name, row.names=F, col.names=F, quote=F)
                    write.table(all5, betas_all_name, row.names=F, col.names=F, quote=F)
                }
